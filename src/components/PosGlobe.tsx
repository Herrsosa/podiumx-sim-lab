...
type Dot = GlobeLocation & { x: number; y: number; size: number };
...
export function PosGlobe({ locations, className = '' }: PosGlobeProps) {
  ...
  // Stable orthographic projector (centered on 0° / 0°) — memoize with useCallback.
    ...
    (lat: number, lng: number): [number, number] | null => {
      ...
      const visible =
        Math.sin(phi0) * Math.sin(phi) +
          Math.cos(phi0) * cosPhi * Math.cos(lambda - lambda0) >
        0;
  ...
  }, [locations, projectPoint]);

  // Project world features into paths
  ...
  const handleDotHover = (dot: Dot | null, event?: React.MouseEvent<SVGCircleElement>) => {
    if (dot && event) {
