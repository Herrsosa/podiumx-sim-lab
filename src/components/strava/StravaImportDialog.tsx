...
export function StravaImportDialog({ activity, open, onOpenChange, onImported }: StravaImportDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="flex flex-col p-0 gap-0 sm:max-w-2xl h-[100dvh] sm:h-auto sm:max-h-[85vh] sm:rounded-lg">
        ...
        <div className="p-4 border-t bg-background shrink-0 sm:rounded-b-lg">
          <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
            <Button type="button" variant="outline" onClick={() => handleOpenChange(false)} disabled={saving}>
              Cancel
            </Button>
            <Button type="submit" form="strava-import-form" disabled={saving || !userId}>
              {saving ? (
                <>
