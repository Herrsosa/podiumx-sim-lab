-- Fix marketplace pct_change_24h so it is not always 0 when the first on-chain trade
-- happens within the last 24 hours (and there is no trade older than 24 hours).
--
-- Approach: prefer a baseline from the latest on-chain trade before the window; if none exists,
-- use the earliest on-chain trade inside the window. This avoids relying on off-chain curve params
-- (a/b/c) which may diverge from the on-chain contract configuration.
...
comment on view public.athlete_metrics_24h is 'Aggregated 24h metrics and 7d sparkline data for marketplace display (on-chain only, with trade-based baselines)';
