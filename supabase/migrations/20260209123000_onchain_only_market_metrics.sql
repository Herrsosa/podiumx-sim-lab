-- Make marketplace metrics reflect on-chain market only.
--
-- Why: if off-chain/virtual trades are still written to public.trades, they will desync the UI from
-- the on-chain bonding curve. Filtering on `is_on_chain` makes the DB a read model of chain activity.
...
-- On-chain only movers over a window.
...
comment on function public.get_top_movers(interval, int) is 'Compute movers over a window (on-chain trades only)';
